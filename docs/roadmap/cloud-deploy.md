...
| 24 | Dashboard + settings (plan M11): `fieldSuggestions.ts` pure ranker over `field_suggestion_inputs()` (k-anonymity floor 3 tenants, `log1p(tenants)·unanswered/forms`, own events 3/app cap 5, pins `priority/10`, seeded rules `2 + priority/10` via the predicate DSL, `label:` signals need a pin or own event ⇒ custom screener `q_<fp12>`, answered targets excluded, dedupe by target, templated why-lines); `dashboard/applySuggestion.ts` (inline only for plain profile text + screener answers, everything else deep-links; blank never writes; self-ID never inline); `SuggestedPanel`, `NeedsYouPanel` (realtime `postgres_changes` on `handoff_tasks` + manual refresh; migration `20260912000100` publishes `handoff_tasks`/`engine_status`/`engine_jobs` idempotently), `ApplicationsPanel` + `ApplicationSheet` (`lib/appStatus.ts` vocabulary, tabs, table/cards, receipt Dialog from a signed URL, `/dashboard/applications/:id`), `ReferralDrafterPanel` (`LockedPanel` naming the refusing gate; unlocked lists `outreach_drafts`), `EngineLine` + pause, `SettingsPage` (`/settings`: pause via `user_engine_controls` update-then-insert, disconnect JobRight, clear self-ID, two-click confirms), `InvitePanel` restyled; migration `20260912000200_field_suggestion_inputs_v2` fixes the applied RPC's `cardinality(jsonb)` call and its empty-container answered-ness and adds `answered_preferences` | UNIT_CONFIRMED (`field-suggestions.test.ts` 12 cases incl. the v2-migration text pin, `apply-suggestion.test.ts`) + FIXTURE_CONFIRMED (walk `artifacts/qa-frontend/2026-09-12-m11`, 134 scenes, 0/0). Both new migrations UNAPPLIED (`SUPABASE_SYNC_ENABLED` off) — `field_suggestion_inputs()` live is UNVERIFIED and known-broken until v2 lands; realtime falls back to manual refresh |
| 25 | Tenancy flags (plan M12): `TENANT_ENGINE_ENABLED` (needs `SUPABASE_URL` + service key; `TENANTS_ROOT`, `TENANT_MAX_CONCURRENT` 1–8) and `REMOTE_BROWSER_ENABLED` (needs `BROWSERBASE_API_KEY` + project id), both fail-closed and refusing to boot without their inputs; `assertSyncConfigured(config, {userId?})`, `runSupabaseSync` / `runReceiptsPush` take the user the rows belong to (the operator's `SUPABASE_SYNC_USER_ID` is needed only when none is passed); registered in CLAUDE.md + mirror, `.env.example`, operator guide, knowledge-graph flags, `fillEnvIsolation` | UNIT_CONFIRMED (`config-tenant-flags.test.ts`: defaults, refusals, bound, registry presence, user-id override, fake-client sync carries the passed id and never the operator's) |
| 26 | Tenant crypto (plan M13): `src/tenants/paths.ts` (`assertTenantId` canonical-uuid-or-refuse, `tenantPaths` — every path under `<TENANTS_ROOT>/<uuid>/`, `isInsideTenant`), `keys.ts` (`deriveTenantKey` = HKDF-SHA256(master, salt uuid, info `dispatch/tenant-candidate-key/v1`) → 32 bytes), `secrets.ts` (`sealSecret`/`unsealSecret` AES-GCM under the tenant key, closed name shape; `writeUnsealed`/`wipeUnsealed` for run-time plaintext); `sensitiveCrypto.ts` key-provider seam `CANDIDATE_KEY_PROVIDER=dpapi\|env\|tenant` (absent ⇒ the pre-M13 path byte-for-byte; `tenant` needs `TENANT_USER_ID` and derives from the DPAPI-wrapped master under `TENANTS_ROOT`, `resolveTenantMasterKey`); the two env names added to `fillEnvIsolation` | UNIT_CONFIRMED (`tenant-crypto.test.ts` 9 tests: uuid guard, path containment, HKDF determinism/distinctness/one-way, cross-tenant blob refused, provider selection + refusals, seal/unseal round-trip, unsealed wipe) |
| 27 | Tenant materializer (plan M14): `src/cloud/tenantMaterializer.ts` (pure cloud rows → the engine's own files: public profile validated by its schema with nothing invented, screener bank + unambiguous profile mirrors, about-me + generated facts, persona or reason, self-ID plaintext → `answer`/`Decline to self-identify`/`""`, own-uid documents in the kind's bucket, education policy, manifest with `outreach_eligible = premium && gmail connected`); `src/tenants/workspace.ts` `materializeWorkspace` (behind `TENANT_ENGINE_ENABLED`, idempotent, documents once per upload, self-ID written ONLY as `sensitive-profile.enc` under the tenant key, consent withdrawn ⇒ removed) + `inspectWorkspace` / `listWorkspaces`; `npm run tenant:materialize` / `tenant:status` (`src/tenants/cli.ts`); `makeSyncClient` exported from `syncSupabase.ts` | UNIT_CONFIRMED (`tenant-materializer.test.ts` 11, `tenant-workspace.test.ts` 4: the engine's loaders accept every written file, no plaintext file carries a self-ID value, wrong tenant key refused, flag-off refusal). LIVE_READ_ONLY against a throwaway onboarded user still UNVERIFIED (flags off) |
| 28 | `tenant:run` (plan M15): `src/tenants/run.ts` `runTenantJob` — gate → materialize → quota budget (`quota.ts`: cloud `user_quota_status` minus local COMPLETED rows the mirror has not counted; missing row ⇒ 0) → sealed JobRight state required, else a `jobright_connect` handoff and a parked heartbeat (no login page is ever opened) → unseal to `private/auth/jobright.storage.json` → spawn `auto:cycle --no-update --defer-gmail --duration --max-apps --max-submits --app-deadline` as a child (`childEnv.ts`: the console ceiling narrowed — autopush / agent leg / CDP autolaunch / extension fill / Outlook / triage act / board discovery / Gmail drafting+verification forced off until M19, operator secrets + tenancy switches + session-mode overrides stripped, workspace paths, `CANDIDATE_KEY_PROVIDER=tenant`, unattended submits = the budget), hard timeout + `taskkill /T /F` → every plaintext wiped in `finally` → status + receipts sync for the tenant's own id → `deriveHandoffsFromRun` (`handoff.ts`, from the tenant's review items + run codes) → `runs/<job>/result.json` → `complete_engine_job`; `context.ts` `currentTenant()`; `npm run tenant:run` | UNIT_CONFIRMED (`tenant-run.test.ts` 9 tests: flag-off refusal, quota-missing ⇒ no child, no sealed session ⇒ handoff + failed-retryable, happy path with the exact child spec — plaintext present only during the child, env below the ceiling with every forced-off and strip, completion counts, ats_login + jobright_reconnect handoffs — timeout path, unsupported kinds dead, ceiling property over every gated key, budget arithmetic, currentTenant). LIVE_READ_ONLY (operator as tenant zero, `SUBMIT_ENABLED=false`) UNVERIFIED: needs a sealed JobRight state (M17 capture) and the flags on |
| 29 | Spike (plan M16): can a hosted user sign in to JobRight inside a browser we host? Harness built — `src/browser/remoteBrowser.ts` provider seam, `src/auth/cdpPolicy.ts`, `npm run remote:probe` (create → LIVE VIEW → sign in → attach through the session seam → validate → capture to `private/cloud/spike/` → release, JSON evidence); report with the results table and the three hedges in `docs/roadmap/browserbase-spike-2026-09-14.md` | UNIT_CONFIRMED (provider against a fake fetch, policy); the live questions (Google challenge, headless validity of the captured state, proxy need, session lifetime) are UNVERIFIED — no Browserbase account on this box; the report says exactly how to fill them in |
| 30 | Handoff engine side (plan M17): `assertCdpUrlAllowed` in `PlaywrightServiceSession`'s CDP_ATTACH path (loopback always; remote only behind `REMOTE_BROWSER_ENABLED`, refused by name); `src/tenants/capture.ts` `provisionHandoff` / `captureHandoff` / `resolveJobrightAuthParks` / `assessPremiumText`; `src/tenants/reconnect.ts` `runReconnectVerify` wired as `tenant:run --kind reconnect_verify --task <id>` (task must be the tenant's own user_done jobright_connect/reconnect; connect URL derived from the provider session id, never stored; capture ⇒ sealed `secrets/jobright.storage.enc` + `engine_set_integration_status connected` (premium promoted only on evidence) + parks resolved and requeued through the legal edge; otherwise the task reopens with the reason while attempts < 3, then fails). `ats_login` / `captcha` tasks are created and shown (M15) — their remote-browser resolution is a later milestone | UNIT_CONFIRMED (`tenant-handoff.test.ts` 9, `tenant-run.test.ts` +2: the end-to-end reconnect through `runTenantJob` with fake provider + session, the refusal without a browser; both service-session seam tests green with the policy in place). LIVE_READ_ONLY (own storageState captured via the provider; headless open validates) UNVERIFIED — needs the Browserbase account and `REMOTE_BROWSER_ENABLED` |
| 31 | `tenant:scheduler` (plan M18): `src/tenants/scheduler.ts` — `planApplyJobs` (pure; connected JobRight + quota remaining + not paused + no blocking JobRight handoff + no active apply + none in the last 60 min ⇒ one apply job, ≤ 5 per tick, every refusal named), `schedulerTick` (reap expired leases → handoff poller: `requested` JobRight tasks provisioned on the remote browser, `live` past `expires_at` expired + provider session released, `user_done` ⇒ a `reconnect_verify` job with the task id, other kinds left visible → planner → lease ≤ `TENANT_MAX_CONCURRENT` of `apply`/`reconnect_verify` → `runTenantJob` each), `makeSchedulerIo` (every cloud query in one seam; the planner inserts `engine_jobs` rows directly and reads the partial-unique-index refusal as "already active"), `runTenantScheduler` (bounded by duration AND a tick cap; SIGINT stops leasing after the current tick); `npm run tenant:scheduler -- [--duration] [--interval] [--max-concurrent] [--once]` | UNIT_CONFIRMED (`tenant-scheduler.test.ts` 6 tests: planner refusals + cap, a full tick against a fake io/provider/runner, idempotent enqueue + a throwing job recorded, flag-off refusal, `--once` and the tick cap with a frozen clock). LIVE_MUTATION (tenant zero: one apply job, `max-submits 1`, receipt lands) UNVERIFIED — needs the flags on and a sealed JobRight state |
| 32 | Per-user Gmail, drafts only (plan M19): `readonlyGuards.ts` admits the compose scope deliberately (decision 2026-09-11) and bans every send endpoint (messages/drafts, dotted/slashed) + send/modify scopes; `tokenStore.ts` v2 `scopes[]` (readonly required, v1 parses); `accessToken.ts` shared refresh + scope assertion; `oauthExchange.ts` (PKCE code exchange with the engine's Web client — refuses wider / readonly-less / offline-less grants); `draftsApi.ts` (exactly `POST users/me/drafts` + `GET users/me/drafts/{id}`, asserted per request; RFC 822 with attachments; DRAFT-label read-back, SENT refused); migration `20260914000100_gmail_oauth` (`gmail_oauth_requests`, RPC-insert only + `submit_gmail_oauth_code` enqueuing `gmail_exchange`); `src/tenants/gmailExchange.ts` wired as `tenant:run --kind gmail_exchange` (cloud ciphertext without the client secret, sealed workspace copy, request deleted, stale ⇒ disconnected with reason) and leased by the scheduler; `outreachMirror.ts` → `outreach_drafts` after each run; `candidateIdentity()` replaces the name/LinkedIn literals in outreach validation; SPA: `gmailOauth.ts` (PKCE S256, scopes drift-tested vs the guard), `/gmail/callback`, IntegrationsStep Gmail card (refuses by name without `VITE_GMAIL_OAUTH_CLIENT_ID`), `submitGmailOauthCode`; `GMAIL_OAUTH_*` settings; operator guide §28 | UNIT_CONFIRMED (`gmail-drafts-api.test.ts` 11, `gmail-oauth-pkce.test.ts` 2, `tenant-run.test.ts` +1; the existing Gmail/broker/guard tests green with the widened set) + `frontend:typecheck`/`frontend:build`. Migration UNAPPLIED (flags off); LIVE_MUTATION (one API draft in a test mailbox, read back, never sent) UNVERIFIED — needs the Web OAuth client (restricted scopes: Testing mode, 7-day tokens) and the outreach pipeline's transport switch for tenant children (follow-up; `gmailDrafts.ts` untouched this session) |
| 33 | Field-signals push + pins CLI (plan M21): `src/cloud/fieldSignals.ts` — `selectTenantFieldSignals` (fill_field_outcomes ⋈ fill_runs + screener_predictions, grouped by (signal_key, ats): canonical keys or 12-hex label fingerprints; forms seen / unanswered / required / first+last seen), `selectUserFieldEvents` (one per (field, application, kind) from skip rows + "Answer needed:" review items), `usableSignalLabel` (isDemographicsField + SENSITIVE_QUESTION + page widgets + unusable + < 8 chars), exact `FIELD_SIGNAL_COLUMNS` / `FIELD_EVENT_COLUMNS` whitelists, `runFieldSignalsPush` (absolute upserts through `engine_upsert_field_signals/events` in batches of 200) — inside `cloud:sync` (`--no-signals`) with the operator's id and after every tenant run with the tenant's; `npm run cloud:field-pins -- list\|add\|disable` (`src/cloud/fieldPinsCli.ts`, service role behind `SUPABASE_SYNC_ENABLED`, keys/stores validated against the vocabulary) | UNIT_CONFIRMED (`cloud-field-signals-push.test.ts` 3 tests: the label gate, aggregation with the exact column sets and no value-bearing key — `expected_*`, `observed_*`, `value_fingerprint`, `selected_option`, `options_sample_json`, `prediction_json` never appear —, events one per (field, app, kind), the push through the two RPCs with the tenant id + the sync-gate refusal). LIVE_MUTATION (signals visible in `field_signals`; the `runProfilesPull` snapshot stays EEO-free) UNVERIFIED — flags off |
| 34 | Feed sample + auth-expiry → handoff → resume (plan M20): `src/tenants/feedSample.ts` `runFeedSample` (sealed session required, else `jobright_connect`; unseal → `feedSampleCli.ts` child with every gated flag off + `DRY_RUN` → wipe → titles/companies/locations only to `jobright_feed_samples`; auth wall ⇒ `jobright_reconnect` + `engine_set_integration_status(jobright, expired)`), wired as `tenant:run --kind feed_sample` and leased by the scheduler (the user's `request_engine_job('feed_sample')` reaches it); after any apply run that derives `jobright_reconnect` the integration is marked `expired`, the scheduler stops planning for that user (blocking handoff), `reconnect_verify` seals a new session, resolves the parks and requeues — the resume | UNIT_CONFIRMED (`tenant-feed-sample.test.ts` 3: handoff without a session, the exact child spec with every flag off and plaintext only during the child, titles-only rows, the auth-wall path). FIXTURE (a real headless open of a recorded feed) and LIVE_READ_ONLY UNVERIFIED — needs a sealed tenant session |
